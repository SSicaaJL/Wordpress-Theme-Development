...
function nusscistudentlife_comment($comment, $args, $depth){ ?>
	<div class="d-flex my-3 m-2" id="div-comment-<?php comment_ID() ?>"> <!-- Creating a Flex Box -->
		...
		<div> 
			<div style="font-size: 0.9rem;"> <!-- The commentor's name and date -->
			...
			<?php if ( is_user_logged_in() ) :?>
				<div class="reply" style="font-size: 0.9rem;"> <!-- A responsive reply to be used with comment threading script in function.php -->
					<?php 
						comment_reply_link( 
							array_merge( 
								$args, 
...
<?php if ( is_user_logged_in() ) { // If is logged in, activate the comment form
	...
	<div class="my-3" style="font-size: 0.9rem;"> <!-- The top part of comment section, containing user name and number of comments -->
	...
	comment_form( array( // The comment form, using Wordpress' inbuilt comment form
		...
		'title_reply'			=> '',
		'title_reply_to'		=> '',
		'comment_field'			=> '<textarea id="comment" name="comment" class="form-control sci-comment-box" rows="3"></textarea>',
		'label_submit'			=> 'Post',
		'class_submit'			=> 'float-right mt-2 py-0 px-2 sci-comment-post',
